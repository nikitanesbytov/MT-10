...
class RollingMillSimulator(RollingMill):
    ...
    def save_logs_to_file(self, filename="rolling_log.txt"):
        ...
        with open(filename, 'w') as f:
            # Определяем ширину столбцов (такая же, как в print_logs)
            col_widths = {
                'Time': 8,
                'Gap': 8,
                'Speed_V': 6,
            ...
            for i in range(len(self.time_log)):
                row = [
                    ...
                    # f"{self.pyromet_1[i]:<{col_widths['Pyro1']}.1f}",
                    # f"{self.pyromet_2[i]:<{col_widths['Pyro2']}.1f}",
    ...
    def _update_logs(self, time, gap, speed_V, temp, pos_x,pos_x1, speed_V0, speed_V1, length):
        "Обновление внутренних логов без записи в файл"
        self.time_log.append(time)
        self.gap_log.append(gap)
        self.speed_V.append(speed_V)
        # self.pyromet_1.append(pyromet_1)
        # self.pyromet_2.append(pyromet_2)
    ...
    def _simulate_approach_to_rolls(self, n):
        ...
        CurrentS = self.CurrentS[-1] if self.CurrentS else 0
        ...
        phase_time = start_time + time_gap
        # Фаза 1: Выставление зазора валков
        while current_time < phase_time:
            ...
            self._update_logs(current_time, CurrentS, 0, current_temp, self.x_log[-1],0, speed_V0, speed_V1, current_length)

        phase_time = start_time + time_gap + time_accel
        # Фаза 2: Разгон валков
        while current_time < phase_time:
            current_speed = min(current_speed + self.accel * self.time_step, self.V_Valk_Per[n])
            current_time += self.time_step
            current_temp = max(current_temp - temp_drop_per_ms, final_temp)
            self._update_logs(current_time, CurrentS, current_speed, current_temp, self.x_log[-1],0, 0, 0, current_length)
        
        phase_time = start_time + time_gap + time_accel + time_move
        ...
        while current_time < phase_time:
            ...
            current_time += self.time_step
            current_temp = max(current_temp - temp_drop_per_ms, final_temp)
            self._update_logs(current_time, CurrentS, current_speed, current_temp, current_pos_x,current_pos_x1, speed_V0, speed_V1, current_length)
